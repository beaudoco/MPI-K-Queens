...
void findKQueen(int board[][N], int col, int argc, char* argv[])
{
    ...
    if (col != 1) {
        ...
        //FINALIZE OUR USE OF MPI WHEN WE ARE DONE USING
        if (col == 0)
        {
            MPI_Finalize();
        }
    ...
    //WE ARE IN THE SECOND COL LETS USE MPI FOR MASTER-SLAVE
    if (col == 1) {
        ...
        if (my_rank != MASTER) {
            ...
            //WE ARE DONE, LET THE MASTER PROCESS KNOW AND WAIT
        ...
        //MASTER PROCESS
            ...
            //LETS GET THE MESSAGES FROM EVERYONE AND WAIT FOR THAT TO HAPPEN
